...
#define BLYNK_FIRMWARE_VERSION "231120"
#define APP_DEBUG
//-----------------------------
...
float clo_cache = 0;
uint32_t timestamp, status_volume;
...
void rualoc() {
  if (statusRualoc == HIGH) {
    pcf8575_1.digitalWrite(pinvandien, !statusRualoc);
    timer1.setTimeout(long(data.time_run_nenkhi * 1000), []() {
      pcf8575_1.digitalWrite(pinvandien, HIGH);
    });
  }
}
void oncap1() {
  RelayState2 = HIGH;
  pcf8575_1.digitalWrite(pincap1, RelayState2);
...
void onbom1() {
  RelayState = HIGH;
  pcf8575_1.digitalWrite(pinbom1, !RelayState);
...
void onbom2() {
  RelayState1 = HIGH;
  pcf8575_1.digitalWrite(pinbom2, !RelayState1);
...
void rtctime() {
  //Status module
  if (((millis() - status_volume) > 60000) && (status_volume != 0)) {
    status_volume = 0;
    Blynk.logEvent("offline_", String("Module Volume Trạm 4 mất kết nối..."));
  }
  //---------------------
...
BLYNK_WRITE(V10)  // String
{
  ...
  if (dataS == "M") {
    ...
    trip3 = false;
    trip4 = false;
    pcf8575_1.digitalWrite(pincap1, HIGH);
...
BLYNK_WRITE(V32) {  //Nhập số lượng Clo
  if (param.asFloat() > 0) {
    terminal_luuluong.clear();
...
BLYNK_WRITE(V33) {  //Check lại thời gian châm lần cuối
  if (param.asInt() == 1) {
...
BLYNK_WRITE(V34) {  //Status Volume Module
  status_volume = millis();
}
BLYNK_WRITE(V37) {  //Giá trị lưu lượng giếng tạm thời
  var_LLG = param.asInt();
}
//-------------------------------------------------------------------
...
void setup() {
  ...
  pcf8575_1.pinMode(2, OUTPUT);
  pcf8575_1.digitalWrite(2, HIGH);
  pcf8575_1.pinMode(0, OUTPUT);
  pcf8575_1.digitalWrite(0, HIGH);
  pcf8575_1.pinMode(9, OUTPUT);
  pcf8575_1.digitalWrite(pinbom2, HIGH);  // Bom 2
  pcf8575_1.pinMode(11, OUTPUT);
  pcf8575_1.digitalWrite(pinvandien, HIGH);  // Rua loc
  pcf8575_1.pinMode(13, OUTPUT);
  pcf8575_1.digitalWrite(pinbom1, HIGH);  // Bom 1
  pcf8575_1.pinMode(14, OUTPUT);
  pcf8575_1.digitalWrite(pincap1, HIGH);  // Gieng
